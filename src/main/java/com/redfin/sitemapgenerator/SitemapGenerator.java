...
abstract class SitemapGenerator<U extends ISitemapUrl, THIS extends SitemapGenerator<U, THIS>> {
    ...
    /**
     * Add one URL of the appropriate type to this sitemap. If we have reached the maximum number of URLs, we'll throw
     * an exception if {@link #allowMultipleSitemaps} is false, or else write out one sitemap immediately.
     *
    ...
    public THIS addUrl(final U url) {
        if (finished) {
            throw new RuntimeException(
                "Sitemap already printed; you must create a new generator to make more sitemaps");
        }

        UrlUtils.checkUrl(url.getUrl(), baseUrl);
        if (urls.size() == maxUrls) {
            if (!allowMultipleSitemaps) {
                throw new RuntimeException("More than " + maxUrls
    ...
    /**
     * Add multiple URLs of the appropriate type to this sitemap, one at a time. If we have reached the maximum number
     * of URLs, we'll throw an exception if {@link #allowMultipleSitemaps} is false, or write out one sitemap
     * immediately.
     *
     ...
     * @return  this
     */
    public THIS addUrls(final String... urls) {
        for (String url : urls) {
            addUrl(url);
        }
     ...
     * @return  this
     */
    public THIS addUrl(final String url) {
        U sitemapUrl;
        try {
    ...
    /**
     * Write out remaining URLs; this method can only be called once. This is necessary so we can keep an accurate count
     * for {@link #writeSitemapsWithIndex()}.
     *
    ...
    /**
     * Writes out the sitemaps as a list of strings. Each string in the list is a formatted list of URLs. We return a
     * list because the URLs may not all fit -- google specifies a maximum of 50,000 URLs in one sitemap.
     *
    ...
    private void writeSiteMapAsString(final StringBuilder sb, final List<U> urls) {
    ...
    /**
     * After you've called {@link #write()}, call this to generate a sitemap index of all sitemaps you generated. The
     * sitemap index is written to {baseDir}/sitemap_index.xml
     */
     ...
     */
    public File writeSitemapsWithIndex(final File outFile) {
        prepareSitemapIndexGenerator(outFile).write();
        return outFile;
    }

    private SitemapIndexGenerator prepareSitemapIndexGenerator(final File outFile) {
        if (!finished) {
            throw new RuntimeException("Sitemaps not generated yet; call write() first");
        }

        SitemapIndexGenerator sig;
        sig = new SitemapIndexGenerator.Options(baseUrl, outFile).dateFormat(dateFormat).autoValidate(autoValidate)
                                                                 .build();
